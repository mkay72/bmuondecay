...
C  ****  Maximum radial grid dimension.
      INTEGER*4, PARAMETER :: NDIM=50000
C  ****  Maximum number of terms in asymptotic series.
      INTEGER*4, PARAMETER :: MNT=80
...
C
      ZINF=RV(NV)
c       TOL=MAX(ABS(ZINF)*1.0D-10,1.0D-10)
c force to remove the coulomb asymptotic patching
c since we are using VINT for extrapolating
c the wave functions
      TOL=MAX(ABS(ZINF)*1.0D-20,1.0D-40)
      NVI=NV
      DO I=NV,4,-1
...
c since we are using VINT for extrapolating
c the wave functions
c       TOL=MAX(ABS(ZINF)*1.0D-90,1.0D-90)
c       NVI=NV
c       DO I=NV,4,-1
c         IF(ABS(VT(I-1)-ZINF).GT.TOL) THEN
c           NVE=I
c           IF(RT(I)-RT(I-1).LT.EPS*MAX(ABS(RT(I-1)),ABS(RT(I)))) THEN
c             RT(I)=RT(I-1)
c             VT(I)=ZINF
c             NVE=I-1
c           ELSE
c             NVI=NVI+1  ! Add a discontinuity.
c             DO J=NVI,NVE+1,-1
c               RT(J)=RT(J-1)
c             ENDDO
c             VT(NVE+1)=ZINF
